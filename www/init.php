...
<div class="warn">
  <b>⚠ Caddy + uri strip_prefix:</b> APP_URL записан как <code><?= htmlspecialchars($appUrl) ?></code> —
  <b>без slug-префикса пути</b>. Caddy снимает префикс до PHP, поэтому SCRIPT_NAME
  приходит без него. Исправьте вручную в <code>env.php</code>:<br><br>
  <code>define('APP_URL', '<?= htmlspecialchars($appUrl) ?>/<b>ВАШ-SLUG</b>');</code><br><br>
  Если APP_URL неверный — session-gate будет отклонять все запросы по REFERER-проверке.
</div>

<div class="warn">
  <b>⚠ Удалите этот файл (<code>init.php</code>)</b> — deploy.sh его не копирует,
  но если файл есть в www/, он попадёт на сервер при ручном cp. После настройки env.php
  этот файл на сервере не нужен.
</div>
