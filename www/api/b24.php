 ...
 *
 * KB: rules/b24-marketplace-install-post-no-domain.md (фильтр REFERER + резолв domain).
 */
final class B24 {
    ...
    /**
     * Общая save-policy install POST'а: сохранить токены только при первой
     * установке, формальном (re)install (`INSTALL=Y` / `ONAPPINSTALL`) или
     * истёкшем access — не на каждом open'е случайного юзера. Используется
    ...
    public function maybeSaveTokensFromInstallPost(array $post, ?string $referer = null, bool $requesterIsAdmin = false): array {
        ...
        return ['saved' => $saved, 'isFirst' => $isFirst];
    }
...
/** Новый экземпляр B24 из env.php-креденшлов, без проверки наличия токенов. */
function newB24(): B24 {
    $cid = defined('B24_CLIENT_ID') ? B24_CLIENT_ID : '';
    $sec = defined('B24_CLIENT_SECRET') ? B24_CLIENT_SECRET : '';
    return new B24($cid, $sec);
}

/** Фабрика — глобальный экземпляр для API endpoint'ов, требует уже сохранённых токенов. */
function b24(): B24 {
    $b24 = newB24();
    if (!$b24->hasTokens()) {
