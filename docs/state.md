...
Онбординг пройден, но деплоить рано (см. D-001) — сначала нужно перенести
бизнес-код под сущности `ooordi.bitrix24.ru` и привязать GitHub-репозиторий.
`www/api/dashboard-data.php`/`task-create.php` всё ещё завязаны на
entityTypeId/UF-коды первого клиента («Альфа») — их надо переписать под
`/source/RDI *.md` (entityTypeId 1038/1042/1050/1046 уже известны, но
конкретные UF-коды полей нужно переснять живыми REST-вызовами). Ссылка на
GitHub-репозиторий для этого проекта пока не получена от пользователя.
...
Два блокера перед деплоем (см. D-001): (1) бизнес-код ещё под сущности
первого клиента, нужны реальные UF-коды нового портала; (2) нет ссылки на
